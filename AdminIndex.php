...
if (isset($_POST['add_user'])) {
    ...
    $sql = "INSERT INTO adminregister_db (user_name, password, email) VALUES ('$user_name', '$password', '$email')";
    $conn->query($sql);
}
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $conn->query("DELETE FROM adminregister_db WHERE id=$id");
}
...
if (isset($_POST['edit_user'])) {
    ...
    if (!empty($password)) {
        $conn->query("UPDATE adminregister_db SET user_name='$user_name', password='$password', email='$email' WHERE id=$id");
    } else {
        $conn->query("UPDATE adminregister_db SET user_name='$user_name', email='$email' WHERE id=$id");
    }
}

// FETCH USERS
$result = $conn->query("SELECT * FROM adminregister_db");
?>
...
<table>
    ...
    <?php while ($row = $result->fetch_assoc()): ?>
        <tr>
            ...
            <td>
                <a href="AdminIndex.php?edit=<?= $row['id'] ?>">Edit</a> |
                <a href="AdminIndex.php?delete=<?= $row['id'] ?>" onclick="return confirm('Are you sure?')">Delete</a>
            </td>
        </tr>
    <?php endwhile; ?>
</table>
...
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $edit_result = $conn->query("SELECT * FROM adminregister_db WHERE id=$id");
...
?>
    <div class="add-form">
        ...
        <form method="POST">
            ...
            <button type="submit" name="edit_user">Update Admin</button>
            <!-- Cancel Button -->
